 ...
 * IMPORTANT: Pareng layout (layout that contains the view) needs to be a RelativeLayout or a FrameLayout
 * 
 * @author Philipp Jahoda
 */
public abstract class HideShowView extends FrameLayout {
	...
	/** if set to true, the view will acutally be made invisible once the hiding animation has completed */
	private boolean mMakeInvisible = true;
	
	/** flag that indicates of the onLayout() method has been called to get the current position of the view */
    ...
    /** XML constructor */
    public HideShowView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    
    /** even more awesome constructor */
    public HideShowView(Context context, AttributeSet attrs, int defStyle) {
	...
	/**
	 * set the positions between the view should animate when switching show and hide state
	 * the starting position of the view is not ignored tho
	 * @param xShow
	 * @param yShow
	 * @param xHide
	 * @param yHide
	...
	/**
	 * set the position to which the view should animate from its current position
	 * the starting position is the current position of the view, the other position can be set via this method
	 * @param x
	 * @param y
	 */
	public void setPosition(float x, float y) {
	    ...
	    Log.i(LOG_TAG, "set position: show: " + mShowX + " / " + mShowY + ", hide: " + mHideX + " / " + mHideY + ", shown: " + mIsShown);
	}
	
	/**
	 * hide the view if its currently shown, show
	 * it if its currently hidden
	 */
	public void toggle() {
	...
	/**
	 * set this to true if you want the view to be set invisible once the hide animation is finished, default: true
	 * @param enabled
	 */
    ...
    /**
     * set this to true if you want the view to be draggable on the screen, default: true
     * @param enabled
     */
	...
	/**
	 * show the view if its currently not visible
	 * it is advised to set the isVisible variable to true within this method
	 */
	public void show() {
	    ...
	    if(!mIsShown && !mIsAnimating) {
            ...
            if(Utils.supportsAPILevel(16)) {
                ...
                animate().y(mShowY).setDuration(mHideShowDuration).setInterpolator(mShowInterpolator).withLayer();
                animate().x(mShowX).setDuration(mHideShowDuration).setInterpolator(mShowInterpolator).withLayer();
            }
            else if(Utils.supportsAPILevel(12)) {
                
                animate().y(mShowY).setDuration(mHideShowDuration).setInterpolator(mShowInterpolator);
                animate().x(mShowX).setDuration(mHideShowDuration).setInterpolator(mShowInterpolator);
            }
            ...
            h.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    mIsShown = true;
                    mIsAnimating = false;
                    if(mListener != null) mListener.onShow(HideShowView.this, mShowX, mShowY);
                }
            }, mHideShowDuration);
        }
	}
	...
	/**
	 * hide the view if its currently visible
	 * it is advised to set the isVisible variable to false within this method
	 */
	public void hide() {
	    ...
	    if(mIsShown && !mIsAnimating) {
            ...
            if(Utils.supportsAPILevel(16)) {
                ...
                animate().y(mHideY).setDuration(mHideShowDuration).setInterpolator(mHideInterpolator).withLayer();
                animate().x(mHideX).setDuration(mHideShowDuration).setInterpolator(mHideInterpolator).withLayer();
            }
            else if(Utils.supportsAPILevel(12)) {
                
                animate().y(mHideY).setDuration(mHideShowDuration).setInterpolator(mHideInterpolator);
                animate().x(mHideX).setDuration(mHideShowDuration).setInterpolator(mHideInterpolator);
            }
            ...
            h.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    if(mMakeInvisible) setVisibility(View.INVISIBLE);
                    mIsShown = false;
                    mIsAnimating = false;
                    
                    if(mListener != null) mListener.onHide(HideShowView.this, mHideX, mHideY);
                }
            }, mHideShowDuration);
        }
	}
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if(getLayoutParams() instanceof FrameLayout.LayoutParams) {
                    ...
                    params.setMargins((int) (ev.getRawX() - mOffsetX), (int) (ev.getRawY() - mOffsetY), 0, 0);
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
	    ...
	    if(mNotMeasured) {
	        ...
	        mNotMeasured = false;
	        Log.i(LOG_TAG, "onLayout: left: " + left + ", top: " + top + ", right: " + right + ", bottom: " + bottom);
	...
	private static class Utils {
	    ...
	    /**
	     * checks if the api level of the device is higher than
	     * the level in the parameter, if so return true
	     * @param level
	     * @return
	     */
	    public static boolean supportsAPILevel(int level) {
	        
	        if(getAPILevel() >= level) return true;
	        else return false;
	    }
	}
	
	/**
	 * listener that can be notified when the hiding / showing process has finished
	 * @author Philipp Jahoda
	 */
